...
function filesystemFromBootSector(boot: Uint8Array) {
...
export function App() {
	...
	const runOptionExport = async (
		...
		const createLazyFolderWriter = (writerFolderName: string): NtfsExtractionWriter & { outputSegments: string[]; outputFolder: string } => {
			...
			const outputSegments = outputSegmentsForFolder(outputRoot, outputFolder)
			const writer = createFolderWriter(outputRoot, outputFolder, () => totalBytes, setProgress, signal, onBytesWritten)
			let prepared = false
			const prepare = async () => {
				if (prepared) return
				prepared = true
		...
		const collectOptionVhdSources = async () => {
			optionVhdSourcesPromise ??= (async () => {
				...
				const files = optionFiles.some(candidate => candidate.path === file.path) ? optionFiles : [...optionFiles, file]
		...
		const extractOptionSource = async (
			...
			appendLog(`Detected ${nestedFileSystem} filesystem in ${nestedOptionSource.name}`)
		...
		const expandVhdSource = async (
			...
			try {
				vhdNtfsSource = await openVhdChainNtfsSource([source], { onLog: appendLog })
			} catch (error) {
				...
				appendLog(`Linking VHD ${source.name} with ${candidateVhds.length.toLocaleString()} selected OPTION VHD parent layer(s)`)
				vhdNtfsSource = await openVhdChainNtfsSource([...candidateVhds, source], { onLog: appendLog })
			}
